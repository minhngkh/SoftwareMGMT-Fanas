...
class ApiController {
  ...
  async searchBooks(req, res) {
    ...
    if (!phrase) {
      return res
        .status(400)
        .json({ error: "Query parameter 'phrase' is required." });
    }
  ...
  async filterBooks(req, res) {
    ...
    if (!genre) {
      return res
        .status(400)
        .json({ error: "Query parameter 'genre' is required." });
    }
  ...
  async updateProfile(req, res, next) {
    ...
    const updateInfo = { favoriteGenres };
  ...
  async getFavorite(req, res) {
    ...
    try {
      ...
      const index = favoriteList.indexOf(bookId);
      if (index !== -1) {
        return res.status(200).send("Book has already been in favorite list");
      }
      return res.status(404).send("Book is not found in favorite list");
    } catch {
  ...
  async addFavorite(req, res) {
    ...
    try {
      ...
      const index = favoriteList.indexOf(bookId);
      if (index !== -1) {
        return res.status(409).send("Book has already been in favorite list");
      }

      favoriteList.push(bookId);

      const updateInfo = { favoriteList: favoriteList };
  ...
  async createNewCustomer(req, res, _) {
  ...
  async createReview(req, res) {
    ...
    if (!bookID || !reviewContent) {
      return res
        .status(400)
        .json({ error: "bookID and reviewContent are required." });
    }
  ...
  async getReviewsByBookId(req, res) {
    ...
    try {
      ...
      const foundReviews = await Promise.all(
